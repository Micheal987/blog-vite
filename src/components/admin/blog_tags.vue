...
const isName = (name: string): boolean => {
    let count = tabList.value.filter((item) => { return item.name === name })
    if (count.length > 0) {
        return true
    }
    return false
...
onMounted(()=>{
    //总宽度
    let widthCount = document.querySelector(".mySwiper")?.clientWidth
    //实际宽度
    let actualWidth = document.querySelector(".mySwiper")?.scrollWidth
    if ((widthCount as number) < (actualWidth as number)){
        return
    }
   let allMySwiper = document.querySelectorAll(".swiper-slide") //总个数
   let sum = 0 //计算什么时候大于实际宽度
   let count = 0 //swiper 的总个数
   for (const slider of allMySwiper){
    sum += slider.clientWidth as number
    if(sum > (widthCount as number)){
        break
    }
    count++
   }
